class VinFastDigitalTwin extends HTMLElement {
    ...
    if (rawTrips && Array.isArray(rawTrips) && rawTrips.length > 0) {
      ...
      for (let day in groupedData) {
        ...
        dayTrips.forEach((trip, i) => {
          ...
          if (Array.isArray(trip.route)) {
            trip.route.forEach(pt => { 
              if (Array.isArray(pt) && pt.length > 2 && pt[2] > maxSpeed) maxSpeed = pt[2]; 
            });
          }
          ...
          if (i < dayTrips.length - 1) {
            ...
            if (gapSecs < 900) pauseSecs += gapSecs; 
            else parkingSecs += gapSecs; 
          } else {
            trip.pauseAfter = 0;
          }
        });
        ...
        this._dayStats[day] = {
          ...
          pauseMins: Math.round(pauseSecs / 60), 
          parkingMins: Math.round(parkingSecs / 60), 
  ...
  renderCalendar() {
    ...
    for (let i = 0; i < firstDay; i++) { 
      gridHtml += `<div class="cal-day disabled"></div>`; 
    }
  ...
  _renderHistoryMap(dateStr, tripIndex) {
    ...
    if (tripIndex === 'all') {
      ...
      if (Array.isArray(t.route)) {
        t.route.forEach(pt => { 
          if (Array.isArray(pt) && pt.length > 2 && pt[2] > maxSpd) maxSpd = pt[2]; 
        });
      }
    }
    ...
    tripsToRender.forEach((segmentObj, index) => {
      ...
      if (index < tripsToRender.length - 1 && tripIndex === 'all') {
        ...
        let marker = L.marker(latlngs[latlngs.length - 1], { 
          icon: L.divIcon({ className: iconClass, html: currentStopNum }) 
        }).addTo(this._historyLayerGroup);
      ...
      if (!hasSensorData) {
        ...
        if (foundData) {
          ...
          for (let key in speedBands) {
            if (speedBands[key] > maxDist) { 
              maxDist = speedBands[key]; 
              bestBand = key; 
            }
          }
        ...
        bars.forEach(b => {
          ...
          htmlChart += `
            <div style="display:flex; align-items:center; gap:8px;">
              <div style="width:35px; font-size:10px; text-align:right; font-weight:bold; color:var(--secondary-text-color, #475569);">${b.label}</div>
              ...
              <div style="width:40px; font-size:10px; font-weight:bold; color:var(--primary-text-color, #1e3a8a);">${displayVal}</div>
            </div>
          `;
        });
  ...
  updateAllStats() {
    ...
    // HÀM TỰ ĐỘNG TÌM SENSOR
    const findSensor = (keywords) => {
      for (let key in this._hass.states) {
        if (!key.startsWith('sensor.')) continue;
        const lowerKey = key.toLowerCase();
        const allMatch = keywords.every(kw => lowerKey.includes(kw.toLowerCase()));
        if (allMatch) {
          const val = this._hass.states[key].state;
          if (val !== 'unavailable' && val !== 'unknown' && val !== '' && val !== '--') {
    ...
    let totalEnergy = findSensor(['tong', 'dien', 'nang', 'sac']);
    if (totalEnergy === null) totalEnergy = findSensor(['dien', 'nang', 'tong']);
    if (totalEnergy === null) totalEnergy = findSensor(['total', 'energy']);
    if (totalEnergy === null) totalEnergy = 0;
    
    // Lấy tổng quãng đường
    let totalDistance = findSensor(['tong', 'odo']);
    if (totalDistance === null) totalDistance = findSensor(['odo', 'tong']);
    if (totalDistance === null) totalDistance = findSensor(['quang', 'duong', 'tong']);
    ...
    // Tính CO₂ tổng (1 kWh = 0.5 kg CO₂)
    const totalCO2 = totalEnergy * 0.5;
    const totalTrees = Math.round(totalCO2 / 10);
    const fuelSaved = totalDistance * 0.06; // Xe xăng 6L/100km
    ...
    // Ô CO₂ TỔNG
    const totalCo2El = this.querySelector('#vf-stat-total-co2');
    if (totalCo2El) {
      totalCo2El.innerHTML = `${totalCO2.toFixed(0)}<span class="stat-unit">kg</span>`;
    }
    
    // Cập nhật detail panel
    const detailTotalEnergy = this.querySelector('#detail-total-energy');
    const detailTotalDistance = this.querySelector('#detail-total-distance');
    const detailTotalEfficiency = this.querySelector('#detail-total-efficiency');
    const detailTotalCo2 = this.querySelector('#detail-total-co2');
    const detailTotalTrees = this.querySelector('#detail-total-trees');
    const detailTotalFuelSaved = this.querySelector('#detail-total-fuel-saved');
    
    if (detailTotalEnergy) detailTotalEnergy.innerText = totalEnergy.toFixed(1);
    if (detailTotalDistance) detailTotalDistance.innerText = totalDistance.toFixed(1);
    if (detailTotalEfficiency) detailTotalEfficiency.innerText = avgEfficiency.toFixed(1);
    if (detailTotalCo2) detailTotalCo2.innerText = totalCO2.toFixed(0);
    if (detailTotalTrees) detailTotalTrees.innerText = totalTrees;
    if (detailTotalFuelSaved) detailTotalFuelSaved.innerText = fuelSaved.toFixed(1);
  ...
  initMap() {
    ...
    if (zoomCtrl) { 
      zoomCtrl.style.marginTop = '45px'; 
      zoomCtrl.style.marginLeft = '12px'; 
  ...
  set hass(hass) {
    ...
    if (!this.content) {
        ...
        <ha-card class="vf-card">
          <div class="vf-card-container">
            ...
            <div class="vf-stats-grid">
              ...
              <!-- Ô ĐIỆN NĂNG TỔNG (mới) -->
              <div class="stat-box clickable" id="box-total-energy">
                <div class="box-main">
              ...
              <!-- Ô CO₂ TỔNG (mới) -->
              <div class="stat-box clickable" id="box-total-co2">
                <div class="box-main">
                  <ha-icon icon="mdi:tree" style="color: #10b981;"></ha-icon>
                  <div class="stat-info">
                    <div class="stat-label">CO₂ TỔNG</div>
                    <div class="stat-val" id="vf-stat-total-co2">-- kg</div>
                  </div>
                </div>
              </div>
              ...
              <!-- DETAIL CONTAINER CHO 2 Ô MỚI -->
              <div class="stat-detail-container" id="detail-container-4">
                <div class="stat-detail-content" id="detail-total-stats" style="padding: 15px;">
                  <div style="font-weight: bold; margin-bottom: 15px; color: var(--primary-text-color);">
                    <ha-icon icon="mdi:chart-box" style="--mdc-icon-size: 18px; margin-right: 6px;"></ha-icon>
                    Thống kê tổng hợp vòng đời
                  </div>
                  
                  <!-- 2 cột hiển thị thông số -->
                  <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 15px; margin-bottom: 20px;">
                    <!-- Cột Điện năng tổng -->
                    <div style="background: var(--secondary-background-color); border-radius: 12px; padding: 15px; text-align: center;">
                      <div style="font-size: 12px; color: var(--secondary-text-color); margin-bottom: 8px;">⚡ ĐIỆN NĂNG TỔNG</div>
                      <div style="font-size: 28px; font-weight: bold; color: #3b82f6;" id="detail-total-energy">-- kWh</div>
                      <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--divider-color);">
                        <div style="display: flex; justify-content: space-between; font-size: 12px; margin-bottom: 6px;">
                          <span>📊 Quãng đường tổng:</span>
                          <b id="detail-total-distance">-- km</b>
                        </div>
                        <div style="display: flex; justify-content: space-between; font-size: 12px;">
                          <span>⚡ Hiệu suất trung bình:</span>
                          <b id="detail-total-efficiency">-- kWh/100km</b>
                        </div>
                      </div>
                    </div>
                    ...
                    <!-- Cột CO₂ tổng -->
                    <div style="background: var(--secondary-background-color); border-radius: 12px; padding: 15px; text-align: center;">
                      <div style="font-size: 12px; color: var(--secondary-text-color); margin-bottom: 8px;">🌳 CO₂ TIẾT KIỆM</div>
                      <div style="font-size: 28px; font-weight: bold; color: #10b981;" id="detail-total-co2">-- kg</div>
                      <div style="margin-top: 12px; padding-top: 12px; border-top: 1px solid var(--divider-color);">
                        <div style="display: flex; justify-content: space-between; font-size: 12px; margin-bottom: 6px;">
                          <span>🌿 Tương đương cây xanh:</span>
                          <b id="detail-total-trees">-- cây</b>
                        </div>
                        <div style="display: flex; justify-content: space-between; font-size: 12px;">
                          <span>🚗 So với xe xăng (6L/100km):</span>
                          <b id="detail-total-fuel-saved">-- lít</b>
                        </div>
                      </div>
                    </div>
                  </div>
                  
                  <div style="margin-top: 10px; padding: 10px; background: var(--primary-background-color); border-radius: 8px; font-size: 11px; color: var(--secondary-text-color); text-align: center; border: 1px solid var(--divider-color);">
                    💡 1 kWh điện = 0.5 kg CO₂ | 1 cây xanh hấp thụ ~10kg CO₂/năm | Xe xăng tiêu thụ 6L/100km ≈ 0.14 kg CO₂/km
                  </div>
                </div>
              </div>
            </div> 
      ...
      this.toggleExpand = (boxId, detailId, containerId) => {
        ...
        if (!isExpanded) {
          ...
          if (boxId === '#box-total-energy' || boxId === '#box-total-co2') {
            this.updateAllStats();
          }
        }
      };
      ...
      this.renderChargeHistory = () => {
        ...
        if (this._chargeHistoryData && this._chargeHistoryData.length > 0) {
          ...
          if (this._chargeHistoryData.length > 3) {
            html += `<div style="padding:8px 0; text-align:center; font-size:10px; color:#64748b;">+ ${this._chargeHistoryData.length - 3} lần sạc khác</div>`;
          }
        } else { 
      ...
      attachClick('#box-total-energy', '#detail-total-stats', '#detail-container-4');
      attachClick('#box-total-co2', '#detail-total-stats', '#detail-container-4');
          ...
          if (confirm("Chạy ngầm AI (Mapbox/Stadia/OSRM) nắn thẳng lộ trình?")) {
            ...
            if (this._hass && this._entityPrefix) {
              this._hass.callService("button", "press", { entity_id: `button.${this._entityPrefix}_fix_map` })
                .then(() => { 
                  alert("Đã gửi lệnh nắn đường! Vui lòng chờ vài phút."); 
                  setTimeout(() => { 
            ...
            setTimeout(() => {
              if (item.lbl && item.lbl.id === 'lbl-eff') {
                ...
                if (this._effToggleState) { 
                  ...
                  item.lbl.style.color = "#3b82f6"; 
                  if (item.icon) { 
                    item.icon.setAttribute('icon', 'mdi:map-marker-distance'); 
                    item.icon.style.color = "#3b82f6"; 
                  } 
                } else { 
                  ...
                  if (item.icon) { 
                    item.icon.setAttribute('icon', 'mdi:battery-charging-60'); 
    ...
    const sensorsToWatch = [
      ...
      { name: "Đèn pha", state: getValidState('trang_thai_den_pha'), icon: "mdi:car-light-high" }, 
    ...
    sensorsToWatch.forEach(s => {
      if (s.state && s.state !== '--' && s.state !== 'unknown') {
        ...
        if (stLower.includes('mở khóa') || stLower.includes('tắt an ninh') || stLower.includes('nhả phanh tay') || (stLower.includes('đang nháy') && s.name === 'Cảnh báo') || (s.name === 'Pin 12V' && parseFloat(s.state) < 40)) { 
          color = '#ef4444'; 
          if (s.name !== 'Điều hòa') warningCount++; 
        } else if (stLower.includes('khóa') || stLower.includes('bật an ninh') || stLower.includes('kéo phanh tay') || (stLower.includes('đang bật') && s.name !== 'Đèn pha')) { 
          color = '#10b981'; 
        } else if (stLower.includes('bật') && s.name === 'Đèn pha') { 
          color = '#f59e0b'; 
        }
        sensorHtml += `
          <div style="display:flex; justify-content:space-between; align-items:center; background:var(--primary-background-color, white); padding:8px 12px; border-radius:8px; border:1px solid var(--divider-color, #e2e8f0);">
    ...
    if (aiContainer && aiTextEl) {
      if (!aiAdvisor || aiAdvisor === 'DISABLED' || aiAdvisor === 'unavailable') {
        ...
        if (this._lastAiMessage !== aiAdvisor) {
          ...
          if (aiContentEl) { 
            aiContentEl.style.maxHeight = '500px'; 
